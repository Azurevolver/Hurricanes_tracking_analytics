...
from datetime import datetime     # Katy
"====================================================================================================================="
...
def process_storm_data(file_name, result_dict):
    ...
    with open(file_name, 'r') as input_file:
        ...
        for line in input_file:
            ...
            current_time = get_time_mins(line)

            # Add up the total times
            time = calculate_the_time(storm_dict, current_time)
            storm_dict[Time_List].append(time)

            # Set the current time
            storm_dict[Current_Time_mins] = current_time

            calculate_the_speed(storm_dict, Distance_List, Time_List)
            # 這裡暫時先call function確認結果，結果無誤再改成寫入txt
...
result_dict = {}
process_storm_data("AtlanticCopy.txt", result_dict)
...
def calculate_the_speed(storm_dict, Distance_List, Time_List):
    speed_list = []
    for d, t in range(len(storm_dict[Distance_List], storm_dict[Time_List])):
        speed_each = storm_dict[Distance_List][d] / storm_dict[Time_List][t]
        speed_list.append(speed_each)

    speed_max = max(speed_list)
    speed_mean = sum(Distance_List) / sum(Time_List)

    return speed_max, speed_mean




def calculate_the_time(storm_dict, current_time):
    last_time = storm_dict[Current_Time_mins]
    if last_time == 0:
        return 0

    time = (current_time - last_time) / 60
    return time



def get_time_mins(line):
    dt = datetime(year = int(line[0:4]), month = int(line[4:6]), day = int(line[6:8]),
                  hour = int(line[10:12]), minute = int(line[12:14]))
    '''
    minutes = 0
    if current_day = last_day:
        minutes = (current_minute - last_minute)
    if current_day != last_day:
        minutes = 24 + (current_minute - last_minute)
    '''
    return minutes
